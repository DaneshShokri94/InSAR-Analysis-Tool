"""
ASF InSAR Product Viewer
========================
Professional viewer for ASF On-Demand InSAR GeoTIFF products.
Supports: wrapped phase, unwrapped phase, coherence, amplitude, DEM, displacement maps.

Author: InSAR Analysis Tool
Version: 1.0
"""
...
class ASFInSARViewer:
    def __init__(self, root):
        ...
        if not HAS_GDAL and not HAS_RASTERIO:
            messagebox.showwarning("Warning", 
                "Neither GDAL nor rasterio found!\n\n"
                "Install with:\n"
    ...
    def create_insar_colormaps(self):
        ...
        # Displacement - blue-white-red (negative to positive)
    ...
    def get_colormap(self, name):
        """Get colormap by name, including custom ones"""
    ...
    def create_widgets(self):
        ...
        ttk.Label(header_frame, text="🛰️ ASF InSAR Product Viewer", style='Title.TLabel').pack(side=tk.LEFT)
        ttk.Label(header_frame, text="GeoTIFF Visualization for ASF On-Demand Products", 
    ...
    def create_folder_card(self, parent):
        body = self.create_card(parent, "📁 INSAR PRODUCT FOLDER")
        
        tk.Label(body, text="Select folder with extracted GeoTIFFs:", 
        ...
        tk.Entry(file_frame, textvariable=self.folder_path, 
                bg=self.colors['bg_light'], fg=self.colors['text'],
                insertbackground=self.colors['text'], relief=tk.FLAT, 
                font=('Segoe UI', 9)).pack(side=tk.LEFT, fill=tk.X, expand=True, ipady=8, padx=(0, 10))
    ...
    def create_info_card(self, parent):
        body = self.create_card(parent, "ℹ️ PRODUCT INFO")
    ...
    def create_visualization_panel(self, parent):
        ...
        self.ax.text(0.5, 0.5, 'InSAR Product Viewer\n\nSelect a folder to begin',
    ...
    def load_folder(self):
        ...
        try:
            ...
            for tif_path in self.tif_files:
                ...
                # Create display name with product type indicator
                if product_type != 'unknown':
                    display_name = f"[{product_type.upper()}] {fname[:40]}..."
                else:
    ...
    def read_geotiff(self, filepath):
        """Read GeoTIFF using GDAL or rasterio"""
        if HAS_GDAL:
            ds = gdal.Open(filepath)
            if ds is None:
                raise Exception(f"GDAL could not open: {filepath}")
            ...
            raise Exception("No GeoTIFF reader available. Install GDAL or rasterio.")
    ...
    def display_product(self, filepath):
        try:
            ...
            title = title_map.get(product_type, fname)
            ...
            cbar_labels = {
                'wrapped_phase': 'Phase (radians)',
                'unwrapped_phase': 'Phase (radians)',
            ...
            # Update title label
            self.title_label.config(text=f"{product_type.upper()} | {data.shape[1]}x{data.shape[0]} pixels")
            
            # Update info
            info_text = f"File: {fname[:35]}...\n" if len(fname) > 35 else f"File: {fname}\n"
            info_text += f"Type: {product_type}\n"
            info_text += f"Shape: {data.shape}\n"
            info_text += f"Data range: {np.nanmin(data):.4f} to {np.nanmax(data):.4f}\n"
            ...
            messagebox.showerror("Error", f"Failed to display product:\n{str(e)}")
        finally:
            self.progress.stop()
    ...
    def compare_products(self):
        """Open comparison view for multiple products"""
        if len(self.tif_files) < 2:
            messagebox.showwarning("Warning", "Need at least 2 products to compare!")
            return
        ...
        tk.Label(select_frame, text="Select products to compare:", 
                bg=self.colors['bg_medium'], fg=self.colors['text'],
                font=('Segoe UI', 11, 'bold')).pack(anchor=tk.W, pady=(0, 10))
        
        # Product checkboxes
        self.compare_vars = []
        for i, tif_path in enumerate(self.tif_files[:8]):  # Max 8 products
            var = tk.BooleanVar()
            fname = os.path.basename(tif_path)
            cb = tk.Checkbutton(select_frame, text=fname[:60], variable=var,
                               bg=self.colors['bg_medium'], fg=self.colors['text'],
        ...
        tk.Button(select_frame, text="Compare Selected", bg=self.colors['accent'],
                 ...
                 command=lambda: self.show_comparison(compare_win)).pack(pady=20)
    ...
    def show_comparison(self, window):
        """Display comparison of selected products"""
        ...
        if len(selected) > 4:
            messagebox.showwarning("Warning", "Maximum 4 products for comparison!")
            selected = selected[:4]
